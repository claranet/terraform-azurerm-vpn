...
resource "azurerm_virtual_network_gateway" "public_virtual_network_gateway" {
  ...
  dynamic "ip_configuration" {
    ...
    content {
      name                 = var.vpn_gw_ipconfig_custom_name == "" ? "${local.vpn_gw_ipconfig_name}-0${ip_configuration.key}" : var.vpn_gw_ipconfig_custom_name
...
resource "random_password" "vpn_ipsec_shared_key" {
  for_each = { for c in var.vpn_connections : c.name => c if nonsensitive(c.shared_key) == null }
  length   = 32
  special  = false
}
