...
| subnet\_gateway | claranet/subnet/azurerm | ~> 9.0 |
...
| nat\_rules | Map of NAT rules to apply to the VPN Gateway. For dynamic NAT rules, if `ip_configuration_name` is not set, the first IP configuration will be used. | <pre>map(object({<br/>    external_mapping = list(object({<br/>      address_space = string<br/>      port_range    = optional(string)<br/>    }))<br/>    internal_mapping = list(object({<br/>      address_space = string<br/>      port_range    = optional(string)<br/>    }))<br/>    mode                  = string<br/>    type                  = optional(string, "Static")<br/>    ip_configuration_name = optional(string)<br/>    })<br/>  )</pre> | `{}` | no |
